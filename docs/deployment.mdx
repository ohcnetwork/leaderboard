...
> **25 MiB file size limit**: Cloudflare Pages only supports individual files up to 25 MiB. If your leaderboard database exceeds this, host it externally (e.g., on GitHub Pages) and configure `data_explorer.source` in `config.yaml` to point to the external URL. See the [Data Explorer docs](/docs/data-explorer#hosting-the-database-on-github-pages) for step-by-step instructions.
