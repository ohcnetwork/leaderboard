...
function generateUsername(): string {
  ...
  switch (style) {
    case 0:
      // firstname-lastname
      return `${faker.person.firstName().toLowerCase()}-${faker.person.lastName().toLowerCase()}`;
    case 1:
      // firstname + number
      return `${faker.person.firstName().toLowerCase()}${faker.number.int({ min: 1, max: 999 })}`;
    default:
...
export function generateContributor(): Contributor {
  ...
  const joining_date = faker.date.between({ from: twoYearsAgo, to: sixMonthsAgo }).toISOString();
