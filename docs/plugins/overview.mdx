...
sequenceDiagram
    ...
    Runner->>DB: Insert badge definitions from config
    Runner->>DB: Insert plugin badge definitions
    ...
    Note over Runner,DB: Main leaderboard aggregation runs
    Runner->>Plugin: aggregate(ctx)
    Plugin->>DB: Compute plugin-specific aggregates
    Note over Runner,DB: Badge evaluation phase
    Runner->>DB: Evaluate config badge rules
    Runner->>DB: Evaluate plugin badge rules
```
...
export default {
  ...
  // Optional: Badge definitions and rules
  badgeDefinitions: [],
  badgeRules: [],
...
- **aggregate** (`function`): Plugin-specific aggregate computation (runs after main aggregation)
- **badgeDefinitions** (`BadgeDefinition[]`): Badge definitions inserted during setup phase
- **badgeRules** (`BadgeRuleDefinition[]`): Badge rules evaluated during evaluate phase (after standard rules)
