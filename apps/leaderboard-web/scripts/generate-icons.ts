...
import { getConfig } from "@/lib/config/get-config.js";
import type { SocialProfileConfig } from "@/lib/config/schema.js";
import fs from "fs";
...
async function getIconNames() {
  ...
  const activityIcons = result.rows
    .map((row: { icon?: string }) => row.icon)
  ...
  const socialProfileIcons = config.leaderboard.social_profiles
    ? (
        Object.values(
          config.leaderboard.social_profiles
        ) as SocialProfileConfig[]
