...
Profiles live in the data repository as Markdown files with YAML frontmatter: `contributors/{username}.md`.

### From the leaderboard site

If your deployment sets `leaderboard.data_source` to a **github.com** repository URL and `leaderboard.data_branch` to the branch where those files are stored (default: `main`), each contributor profile page shows an **Edit profile on GitHub** button. It opens GitHub’s in-browser editor for that user’s file so they can change frontmatter and bio, then commit (or propose a pull request if they don’t have write access).

Configure the branch explicitly when your data repo’s default branch for flat data is not `main` (for example `production`).

### Directly in the data repository

You can always edit `contributors/{username}.md` locally or on GitHub: update the YAML frontmatter fields (`name`, `role`, `title`, `avatar_url`, `social_profiles`, `joining_date`, `meta`, etc.) and the Markdown body (bio). After merging, run your usual data build/import pipeline so the site picks up the changes.
