...
const getLastWeekHighlights = (
  calendarData,
  initialValue = highlightsInitialValue
) => {
  const lastWeek = calendarData.slice(-7);

  const highlights = lastWeek.reduce((acc, day) => {
    return {
...
const getLastMonthHighlights = (calendarData) => {
  // the highlights of a week is passed as initial value to next week and goes
  // on for upto last 4 weeks.
  return getLastWeekHighlights(
    calendarData.slice(-28, -21),
    getLastWeekHighlights(
      calendarData.slice(-21, -14),
      getLastWeekHighlights(
        calendarData.slice(-14, -7),
        getLastWeekHighlights(calendarData.slice(-7))
      )
    )
  );
};
