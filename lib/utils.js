...
export const getWeekDateRangeString = (week) => {
  let today = new Date();
  let lastWeek = new Date(today);
  lastWeek.setDate(today.getDate() - 7 * (getWeekNumber(new Date()) - week));

  let start = String(lastWeek.getDate()).padStart(2, "0");
  start += ` ${months[lastWeek.getMonth()]}`;

  lastWeek.setDate(lastWeek.getDate() + 7);
  let end = `${String(lastWeek.getDate()).padStart(2, "0")}`;
  end += ` ${months[lastWeek.getMonth()]} ${lastWeek.getFullYear()}`;
