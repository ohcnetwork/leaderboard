...
plugin-runner --data-dir=/path/to/data # Custom data directory
```
...
- `global_aggregate` - Organization-level metrics
- `contributor_aggregate` - Per-contributor metrics
- `badge_definition` - Badge types and variants
- `contributor_badge` - Badges earned by contributors
...
### 3. MCP Server (`@leaderboard/mcp-server`)

**Purpose**: Enable AI assistants to query leaderboard data via Model Context Protocol
...
See [MCP Server Documentation](/docs/mcp-server) for detailed usage.
...
- `/badges` - Badge definitions and achievements
- `/data` - [Data Explorer](/docs/data-explorer) (browser-based SQL REPL)
- `/docs` - Documentation
...
5. **Plugin Aggregation Phase**

   ```
   For each plugin (with aggregate method):
     Execute plugin.aggregate(ctx)
     → Compute plugin-specific aggregates
   ```

6. **Badge Evaluation Phase**

   ```
   Evaluate badge rules from config (threshold, streak, growth, composite)
   For each plugin (with badgeRules):
...
8. **Build Phase**
   ```
   Next.js pre-build setup:
     → Copy theme overrides
     → Copy database to public/data.db
     → Copy custom assets from data-repo
     → Download & optimize contributor avatars (WebP, 256×256)
...
Contributor profiles store `avatar_url` pointing to external sources (e.g., GitHub avatars). During the build phase, these are downloaded, converted to WebP, and resized to 256×256 using [sharp](https://sharp.pixelplumbing.com/). The optimized images are saved to `public/avatars/{username}.webp` and served locally, eliminating external dependencies at page load.
...
In addition to the pre-rendered static pages, the database file is shipped as a static asset (`public/data.db`). The [Data Explorer](/docs/data-explorer) page loads this database in the browser via [sql.js-httpvfs](https://github.com/phiresky/sql.js-httpvfs) (SQLite compiled to WebAssembly). A Web Worker makes HTTP range requests to fetch only the database pages needed for each query, enabling ad-hoc SQL queries without any backend.
