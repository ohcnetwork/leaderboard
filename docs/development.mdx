...
1. **Creates directory structure**:
   ...
   │   └── ...
   └── data/
       └── activities/           # Activity JSONL files
           ├── alice-smith.jsonl
           └── ...
   ```
...
- **Roles**: Weighted distribution
  ...
  - 3% bots
  - 2% no role
- **Avatars**: Generated via DiceBear API, then optimized to WebP (256×256) during build
...
**Browser-based (recommended)**: Visit [/data](http://localhost:3000/data) in the dev server to use the built-in [Data Explorer](/docs/data-explorer). It loads the database via WebAssembly and supports arbitrary SQL queries with a schema browser, example queries, and results table.

**CLI**: The database file is at `data/.leaderboard.db` and can be queried with any SQLite tool:
  ...
  async scrape(ctx) {
    ...
    for (const activity of activities) {
      await ctx.db.execute(
        ...
        (slug, contributor, activity_definition, title, occurred_at, points)
        VALUES (?, ?, ?, ?, ?, ?)
      `,
        [
          activity.id,
...
- 🐛 [Report Issues](https://github.com/ohcnetwork/leaderboard/issues)
