...
const additional_consist_eod_points = {
  0: 0,
  1: 0,
  2: 0,
  3: 1,
...
export function getContributorBySlug(file, detail = false) {
  ...
  const weightedActivity = activityData.activity.reduce(
    (acc, activity) => {
      return {
        activity: [...acc.activity, { ...activity, points: points[activity.type] || 0 }],
        points: acc.points + (points[activity.type] || 0),
        comment_created: acc.comment_created + (activity.type === "comment_created" ? 1 : 0),
        ...
        pr_reviewed: acc.pr_reviewed + (activity.type === "pr_reviewed" ? 1 : 0),
        issue_assigned: acc.issue_assigned + (activity.type === "issue_assigned" ? 1 : 0),
        issue_opened: acc.issue_opened + (activity.type === "issue_opened" ? 1 : 0),
      };
    },
    {
      activity: [],
...
export function getContributors(detail = false) {
  const contributors = getContributorsSlugs()
    ...
    .sort((a, b) => {
      return b.weekSummary.points - a.weekSummary.points;
    });
...
export function getCalendarData(activity) {
  ...
  return [...Array(365)].map((_, i) => {
    ...
    const date = new Date(new Date().getTime() - iReverse * 24 * 60 * 60 * 1000);
    // yyyy-mm-dd
    const dateString = `${date.getFullYear()}-${padZero(date.getMonth() + 1)}-${padZero(
      date.getDate()
    )}`;
    const returnable = {
...
export function calculateWeightedWeight(highlightData) {
  let point = 0;
  Object.keys(highlightData).forEach((key) => {
    point += highlightData[key] * (points[key] ?? 0);
  });
...
const getLastWeekHighlights = (calendarData) => {
  ...
  highlights.points =
    calculateWeightedWeight(highlights) + additional_consist_eod_points[highlights.eod_update] ?? 0;
