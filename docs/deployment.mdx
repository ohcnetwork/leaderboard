...
The build pipeline automatically runs these pre-build steps:

1. **Icons** — Generate icon set from configuration
2. **Theme** — Copy custom theme CSS from data repo
3. **Database** — Copy `.leaderboard.db` to `public/data.db` for the Data Explorer
4. **Assets** — Copy custom assets (badges, images) from data repo to `public/assets/`
5. **Avatars** — Download contributor avatars, convert to optimized WebP (256×256), and cache in `public/avatars/`

Avatar images are cached between builds — only new or missing avatars are downloaded, so subsequent builds are fast.
...
> **25 MiB file size limit**: Cloudflare Pages only supports individual files up to 25 MiB. If your leaderboard database exceeds this, host it on [Cloudflare R2](/docs/data-explorer#hosting-the-database-on-cloudflare-r2) (recommended) and configure `data_explorer.source` in `config.yaml` to point to the R2 URL. See the [Data Explorer docs](/docs/data-explorer#using-an-external-database-source) for all hosting options.
