...
export function getContributorBySlug(file, week, detail = false) {
...
export function getContributors(week = -1, detail = false) {
  const contributors = getContributorsSlugs()
    .map((path) => getContributorBySlug(path.file, week, detail))
...
const getLastWeekHighlights = (calendarData, weekNumber = -1) => {
  ...
  let week = 1;
  if (weekNumber !== -1) {
    week = getWeekNumber(new Date()) - weekNumber + 1;
  }
  const lastWeek = calendarData.slice(week * -7).slice(0, 7);
