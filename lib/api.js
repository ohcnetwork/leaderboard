...
/**
 * @param {*} keyForSort value should either be "weekSummary" or "monthSummary"
 */
export function getContributors(detail = false, keyForSort = "weekSummary") {
  const contributors = getContributorsSlugs()
    .map((path) => getContributorBySlug(path.file, detail))
    .sort((a, b) => b[keyForSort].points - a[keyForSort].points);
  return contributors;
}
...
const getCalendarDataBetween = (calendarData, start, end) => {
...
const calendarDataToHighlightsReducer = (acc, day) => {
  return {
...
// Does not include continous weekly EOD updates point bonus.
const getHighlightsBetween = (calendarData, start, end) => {
  return getCalendarDataBetween(calendarData, start, end).reduce(
    calendarDataToHighlightsReducer,
    highlightsInitialValue
  );
};
...
const getLastMonthHighlights = (calendarData) => {
  ...
  return getHighlightsBetween(calendarData, monthAgo, today);
};
