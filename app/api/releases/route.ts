  ...
  `;
    ...
    try {
        ...
        if (!data.organization) {
            return NextResponse.json({ error: `Organization "${NEXT_PUBLIC_GITHUB_ORG}" not found` }, { status: 404 });
        }

        const repositories: Repository[] = data.organization.repositories.nodes;
        const allReleases: Release[] = [];
        for (const repository of repositories) {
            allReleases.push(...repository.releases.nodes);
        }


        const sortedReleases = allReleases.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        ...
        return NextResponse.json({ error: 'Failed to fetch repositories and releases' }, { status: 500 });
    }
}