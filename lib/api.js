...
export function getContributorBySlug(file, detail = false, week) {
  ...
  return {
    ...
    weekSummary: getLastWeekHighlights(calendarData, week),
    calendarData: detail ? calendarData : [],
    ...data,
  };
}

export function getContributors(detail = false, week = -1) {
  const contributors = getContributorsSlugs()
    .map((path) => getContributorBySlug(path.file, detail, week))
...
const getLastWeekHighlights = (calendarData, weekNumber = -1) => {
  //slice according to the week number
  //leaderboard starts at wednesday and ends at tuesday
  let lastWeek;
  let day = (new Date()).getDay()
  // if day is sun,mon,tues then go back day+5 days , 
  //else go back day-2 days, so if day is thur(4) goback 2 days i.e tues(2)
  day = (day <= 2 && day >= 0) ? (day + 5) : (day - 2)
  if (weekNumber !== -1) {
    const week = getWeekNumber(new Date()) - weekNumber;
    lastWeek = calendarData.slice((week * -7) - day).slice(0, 7);
  } else {
    // slice till last wednesday ,e.g: if day is thursday(4) slice only last 2 day data
    lastWeek = calendarData.slice(-1 * day)
  }
