...
Plugins can set custom aggregate values during the `aggregate()` phase. This phase runs after the main leaderboard aggregation, so standard aggregates (like `total_activity_points` and `activity_count`) are already available when your plugin computes its own.

> **Note:** You can also set aggregate values during `scrape()`, but the `aggregate()` method is preferred because it runs after all data has been collected and standard aggregates have been calculated.

### Using the Plugin Aggregate Method

Implement the optional `aggregate()` method in your plugin to compute custom aggregates:
...
export default {
  ...
  async setup(ctx) {
    // Define custom aggregate definitions
    await contributorAggregateDefinitionQueries.upsert(ctx.db, {
      slug: "pr_merged_count",
      name: "PRs Merged",
      description: "Number of pull requests merged",
  ...
  async aggregate(ctx) {
    // Compute custom aggregates after main aggregation
    ...
    for (const contributor of contributors) {
      ...
      const mergedPRs = activities.filter(
        (a) => a.activity_definition === "pr_merged",
      );
...
graph LR
    ...
    Aggregate --> PluginAgg[Plugin Aggregation]
    PluginAgg --> Badges[Badge Evaluation]
