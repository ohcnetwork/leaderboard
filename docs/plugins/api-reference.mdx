...
- **Type**: `BadgeRuleDefinition[]`
- **Description**: Badge rules that this plugin provides. Evaluated during the evaluate phase, after config badge rules. Each rule's `badgeSlug` must match a badge definition (either from config or from `badgeDefinitions`).
...
- `idx_activity_occurred_at` on `occurred_at`
