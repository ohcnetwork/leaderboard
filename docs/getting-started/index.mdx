...
- `@leaderboard/mcp-server` - MCP server for AI assistant integration
...
Profiles are Markdown files in the data repo (`contributors/{username}.md`). Set `leaderboard.data_source` to your **github.com** repository URL and `leaderboard.data_branch` to the branch that holds those files (default: `main`). Contributor profile pages then include **Edit profile on GitHub**, which opens the file in GitHub’s editor. For a full walkthrough (direct edits, commits, and rebuilds), see [Data management](/docs/data-management#updating-contributor-profiles).
...
│   ├── plugin-dummy/             # Example dummy data plugin
...
- [MCP Server for AI Integration](/docs/mcp-server)
